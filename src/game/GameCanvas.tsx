...
const initial={time:90,ammo:3,zoom:1,breath:0,scan:0,breathing:false,scanning:false,reloading:false,state:'LOADING' as MissionState,fps:0,distance:0,heading:270,models:false,remaining:1,muted:false,exposed:false};
export default function GameCanvas({settings,onResult,onExit,mission}:Props){
 const canvas=useRef<HTMLCanvasElement>(null);const commands=useRef<Commands|null>(null);const [hud,setHud]=useState(initial);const [error,setError]=useState(false);const [flash,setFlash]=useState(false);const [snapshot,setSnapshot]=useState('');const [feedback,setFeedback]=useState<'hit'|'miss'|null>(null);const language=settings.language;const t=(it:string,en:string)=>language==='it'?it:en;
 useEffect(()=>{
 ...
 {feedback&&<div className={`shot-feedback ${feedback}`}>{snapshot&&<img src={snapshot} alt={t('Ultimo colpo','Last shot')}/>}<Crosshair size={28}/><span>{feedback==='hit'?t('BERSAGLIO COLPITO','TARGET HIT'):t('COLPO A VUOTO','MISSED')}</span></div>}
