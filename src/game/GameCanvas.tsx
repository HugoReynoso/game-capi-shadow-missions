...
export default function GameCanvas({settings,onResult,onExit}:Props){
 ...
 update();
 scene.executeWhenReady(()=>{if(!disposed){state=document.hidden?'PAUSED':'PLAYING';update();}});
 ...
 {hud.state==='LOADING'&&<div className="modal"><div className="loading"><Crosshair className="spin"/><span>{t('Preparazione del porto…','Preparing the harbor…')}</span></div></div>}
