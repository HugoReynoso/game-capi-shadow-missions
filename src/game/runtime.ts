...
export interface GameHUD {time:number;ammo:number;zoom:number;breath:number;scan:number;breathing:boolean;scanning:boolean;reloading:boolean;state:MissionState;fps:number;distance:number;heading:number;models:boolean}
export interface Commands {fire:()=>void;zoom:(d:number)=>void;reload:()=>void;breath:()=>void;scan:()=>void;pause:()=>void;stick:(x:number,y:number)=>void}
interface Callbacks {hud:(hud:GameHUD)=>void;result:(result:MissionResult)=>void;flash:(flash:boolean)=>void;feedback:(feedback:'hit'|'miss'|null)=>void}

export function createMissionRuntime(canvas:HTMLCanvasElement,settings:Settings,callbacks:Callbacks){
  let disposed=false;
  const world=createWorld(canvas,settings);
  const {engine,scene,camera,animate}=world;
  const audio=new AudioManager(settings.music,settings.sfx);audio.start();
  let state:MissionState='LOADING',time=0,ammo=3,shots=0,hits=0;
  ...
  function finish(reason:MissionResult['reason']){
    ...
    aim.clear();cinematicTime=0;pendingResult=resultFor(reason,time,shots,hits);
  ...
  function fire(){
    ...
    callbacks.feedback(npc?'hit':'miss');clearTimeout(feedbackTimer);feedbackTimer=setTimeout(()=>!disposed&&callbacks.feedback(null),650);
    if(npc&&npc.alive){hits++;world.hit(npc,time);finish(hitOutcome(npc.type));}
  ...
  const commands:Commands={fire,zoom,reload,breath,scan,pause,stick:(x,y)=>{if(state==='PLAYING')aim.setStick(x,y);}};
  ...
  engine.runRenderLoop(()=>{
    ...
    if(state==='PLAYING'){
      time+=dt;if(reloadUntil&&time>=reloadUntil){ammo=3;reloadUntil=0;}
      aim.update(dt,time,time<breathUntil,settings.reducedShake);animate(time,time<scanUntil);
      if(time>=90)finish('timeout');
    }else if(pendingResult){
      cinematicTime+=dt;animate(time+cinematicTime,false);
      if(cinematicTime>.85){const result=pendingResult;pendingResult=null;callbacks.result(result);return;}
    }
    scene.render();if(time-lastHUD>.12){update();lastHUD=time;}
  });
  world.ready.then(()=>{
    if(disposed)return;
  ...
  if(import.meta.env.DEV){
    Object.assign(window,{__capiDebug:{world,aim,aimAt:(id:string)=>{
