...
test('campaign difficulty increases with bounded timers and reachable target counts',()=>{assert.equal(missions.length,10);for(let i=1;i<missions.length;i++){assert(missions[i].timeLimit<missions[i-1].timeLimit);assert(missions[i].speed>missions[i-1].speed);assert(missions[i].targets>=missions[i-1].targets);assert(missions[i].targets<=3);}assert.equal(missions[9].timeLimit,45);});
